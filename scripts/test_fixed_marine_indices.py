...
def test_marine_indices_fix():
    ...
    if len(present_marine) == 5 and len(present_marine_suffix) == 0:
        print(f"   ✅ Marine indices correctly replaced (no '_marine' suffixes)")
    elif len(present_marine_suffix) > 0:
        print(f"   ❌ Still has '_marine' suffixed indices - fix not complete")
    else:
        print(f"   ⚠️ Missing some marine indices")
