...
# No longer need to manipulate sys.path with proper package structure
...
def calculate_marine_corrected_indices(
    Sxx_power, fn, flim_low, flim_mid, S=-35.0, G=0.0
):
    """
    ...
    if (biophony_energy + anthrophony_energy) > 0:
        corrected_indices["NDSI"] = (biophony_energy - anthrophony_energy) / (
            biophony_energy + anthrophony_energy
        )
    else:
...
def calculate_indices_for_segment(
    ...
    try:
        ...
        verbose_print("    Spectrogram complete, starting temporal indices")
        ...
        verbose_print("    Temporal indices complete, starting spectral indices")
        ...
        verbose_print("    Spectral indices complete, processing marine corrections")

        verbose_print("    Combining indices")
        ...
        if hasattr(temporal_indices, "iloc") and len(temporal_indices) > 0:
            temporal_dict = (
                temporal_indices.iloc[0].to_dict() if len(temporal_indices) > 0 else {}
            )
        ...
        if hasattr(spectral_indices, "iloc") and len(spectral_indices) > 0:
            spectral_dict = (
                spectral_indices.iloc[0].to_dict() if len(spectral_indices) > 0 else {}
            )
        ...
        if calculate_marine:
            verbose_print(
                "    Applying marine corrections to 5 indices (NDSI, BioEnergy, AnthroEnergy, rBA, BI)"
            )
            ...
            verbose_print("    Marine corrections applied - 5 indices replaced")
        else:
            verbose_print(
                "    No marine corrections requested - using standard calculations"
            )
        ...
        verbose_print(f"      ERROR calculating indices for {filename}: {e!s}")
        import traceback
...
def process_single_file_standalone(filename_args):
    ...
    try:
        ...
        if mode in ["manual", "hourly"]:  # Time-based segmentation
            mode_description = (
                "hourly (60-minute segments)"
                if mode == "hourly"
                else f"manual ({time_interval}s intervals)"
            )
            ...
            verbose_print(
                f"    Total samples: {total_samples}, samples per interval: {samples_per_interval}"
            )
            ...
            for start_sample in range(0, total_samples, samples_per_interval):
                ...
                verbose_print(
                    f"    Processing segment {segment_count + 1}: samples {start_sample}-{end_sample}, length {interval_length:.2f}s"
                )

                # Handle short segments (< 1 second)
                if interval_length < 1 and previous_segment_wave is not None:
                    verbose_print(
                        f"    Short segment ({interval_length:.2f}s), concatenating with previous"
                    )
                    ...
                    verbose_print(
                        f"    Short segment ({interval_length:.2f}s), skipping"
                    )
                    continue

                segment_wave = wave[start_sample:end_sample]
                if previous_segment_wave is not None:
                    verbose_print("    Concatenating with previous segment")
                ...
                segment_filename = (
                    f"{os.path.basename(filename)}_segment_{segment_count + 1}"
                )
                ...
                if segment_result:
                    verbose_print(
                        f"    Successfully processed segment {segment_count + 1}"
                    )
            ...
            verbose_print("    Processing entire file in dataset mode (full duration)")
            ...
            if file_result:
                verbose_print("    Successfully processed entire file")
                results.append(file_result)
            else:
                verbose_print("    Failed to process file")
