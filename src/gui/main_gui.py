...
def calculate_marine_indices(Sxx_power, fn, flim_low, flim_mid, S=-35.0, G=0.0):
    ...
    if (bio_energy + anthro_energy) > 0:
        marine_indices["NDSI_marine"] = (bio_energy - anthro_energy) / (
            bio_energy + anthro_energy
        )
    else:
    ...
    if np.any(bio_mask):
        bio_spectrum = (
            np.mean(Sxx_power[bio_mask, :], axis=1)
            if len(Sxx_power.shape) > 1
            else Sxx_power[bio_mask]
        )
        marine_indices["BI_marine"] = np.sum(
            bio_spectrum * np.log10(bio_spectrum + 1e-10)
        )
    else:
...
def process_single_file(filename_args):
    ...
    try:
        ...
        parsed_date, filename_with_numbering = parse_date_and_filename_from_filename(
            filename
        )
        if parsed_date is None:
            print(
                f"    Skipping {os.path.basename(filename)} - invalid filename format"
            )
            return None

        # Load audio file
        wave, fs = sound.load(
            filename=filename, channel="left", detrend=True, verbose=False
        )
        ...
        if mode in ["manual", "hourly"]:  # Time-based segmentation
            ...
            for start_sample in range(0, total_samples, samples_per_interval):
                ...
                if interval_length < 1 and previous_segment_wave is not None:
                    previous_segment_wave = np.concatenate(
                        [previous_segment_wave, wave[start_sample:end_sample]]
                    )
                    continue
        ...
        print(f"    ERROR processing {os.path.basename(filename)}: {e!s}")
        return None


def calculate_indices_for_segment(
    wave, fs, sensitivity, gain, flim_low, flim_mid, calculate_marine, filename
):
    """
    ...
    try:
        ...
        spectral_indices, spectral_indices_per_bin = (
...
def safe_plot_index(df, index_name, ax, mode, position_label):
    ...
    try:
        if index_name in df.columns:
            ...
            print(
                f"  WARNING: Index '{index_name}' not found in data - showing placeholder"
            )
            ax.text(
                0.5,
                0.5,
                f"Index not available:\n{index_name}",
                ha="center",
                va="center",
        ...
        print(f"  ERROR plotting {index_name}: {e!s}")
        ax.text(
            0.5,
            0.5,
            f"Error plotting:\n{index_name}",
...
def run_analysis():
    ...
    sensitivity_for_metadata = (
        sensitivity_var.get() if not sensitivity_placeholder[0] else ""
    )
    ...
    if time_scale == "hourly":
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Please enter a valid integer for time interval (seconds)."
            )
            return
    ...
    if flim_low_input and not flim_low_placeholder[0]:  # User provided actual input
        try:
            ...
            messagebox.showerror(
                "Error", "Invalid anthrophony range. Use format: min,max (e.g., 0,1500)"
            )
            return
    ...
    if flim_mid_input and not flim_mid_placeholder[0]:  # User provided actual input
        try:
            ...
            messagebox.showerror(
                "Error", "Invalid biophony range. Use format: min,max (e.g., 1500,8000)"
            )
            return

    sensitivity_input = sensitivity_var.get().strip()
    if (
        sensitivity_input and not sensitivity_placeholder[0]
    ):  # User provided actual input
        try:
            sensitivity = float(sensitivity_input)
            print(f"Custom sensitivity: {sensitivity}")
        except:
            messagebox.showerror(
                "Error", "Invalid sensitivity value. Must be a number."
            )
            return
    ...
    processing_params = {
        ...
        "sensitivity": sensitivity,
        "gain": gain,
        "calculate_marine": (flim_low_var.get().strip() and not flim_low_placeholder[0])
        or (flim_mid_var.get().strip() and not flim_mid_placeholder[0]),
    }

    # Create full file paths
    full_file_paths = [
        os.path.join(input_folder, filename) for filename in filename_list
    ]
    ...
    if compare_performance:
        ...
        process_files_sequential(
            full_file_paths, processing_params
        )
        ...
        if sequential_time > 0:
            ...
            try:
                from processing.performance_comparison_report import (
                ...
                perf_report_folder = os.path.join(
                    output_folder, "metadata", "performance_reports"
                )
        ...
        start_time = time.time()
        processing_results = process_files_sequential(
            full_file_paths, processing_params
        )
        processing_time = time.time() - start_time
        files_processed = len([r for r in processing_results if r is not None])
    ...
    if files_processed == 0:
        ...
        run_metadata.finish_run(
            success=False,
            error_message="No audio files could be processed successfully",
        )
        messagebox.showerror(
            "Error",
            "No audio files could be processed successfully.\n\nCheck the console for error details.",
        )
        return

    # Process results into DataFrames (replacing the old processing logic)
    debug_print(
        f"[DEBUG] Converting {len(processing_results)} results to DataFrames..."
    )
    result_df, result_df_per_bin = convert_results_to_dataframes(
        processing_results, filename_list, date_list, mode
    )
    ...
    if False:  # This disables the old processing code below
        ...
        for i, filename in enumerate(filename_list, 1):
            ...
            try:
                ...
                for start_sample in range(0, total_samples, samples_per_interval):
                    ...
                    result_df_per_bin = pd.concat(
                        [result_df_per_bin, indices_df_per_bin], ignore_index=True
                    )
        ...
        time_scale == "hourly" or time_scale == "dataset"
        ...
        for i, filename in enumerate(filename_list, 1):
            ...
            progress_percent = 40 + (
                50 * i / len(filename_list)
            )  # 40-90% range for file processing
            show_progress(
                f"Processing file {i}/{len(filename_list)}: {filename[:20]}...",
                progress_percent,
            )
            fullfilename = os.path.join(input_folder, filename)
            ...
            try:
                ...
                print(
                    f"    Data processing in {df_time:.1f}s | Total: {total_file_time:.1f}s"
                )
    ...
    if result_df.empty:
        ...
        debug_print(
            f"[DEBUG] result_df has {len(result_df)} rows and {len(result_df.columns)} columns"
        )
    ...
    if time_scale == "manual":
        for filename in filename_list:
            ...
            time_diff = pd.to_timedelta(
                np.arange(len(filename_rows)) * time_interval, unit="s"
            )
    ...
    if run_id:
        expected_files = [
            (
                os.path.join(output_csv_path, f"{run_id}_Acoustic_Indices.csv"),
                f"{run_id}_Acoustic_Indices.csv",
            ),
            (
                os.path.join(output_figures_path, f"{run_id}_correlation_map.png"),
                f"{run_id}_correlation_map.png",
            ),
            (
                os.path.join(output_figures_path, f"{run_id}_individual_features.png"),
                f"{run_id}_individual_features.png",
            ),
            (
                os.path.join(
                    output_figures_path, f"{run_id}_false_color_spectrograms.png"
                ),
                f"{run_id}_false_color_spectrograms.png",
            ),
        ]
    else:
        expected_files = [
            (
                os.path.join(output_csv_path, "Acoustic_Indices.csv"),
                "Acoustic_Indices.csv",
            ),
            (
                os.path.join(output_figures_path, "correlation_map.png"),
                "correlation_map.png",
            ),
            (
                os.path.join(output_figures_path, "individual_features.png"),
                "individual_features.png",
            ),
            (
                os.path.join(output_figures_path, "false_color_spectrograms.png"),
                "false_color_spectrograms.png",
            ),
        ]
    ...
    if existing_files:
        ...
        if not result:
            ...
            run_metadata.finish_run(
                success=False, error_message="Cancelled by user to avoid overwriting"
            )
            return
        else:
    ...
    try:
        ...
        error_msg = f"Failed to save CSV: {e!s}"
        errors.append(error_msg)
        print(f"  ERROR: {error_msg}")
    ...
    available_indices = [
        col for col in full_df.columns if col not in ["Date", "Filename"]
    ]
            ...
            if name == "indice_six":
                ...
                for alt_index in alternatives:
                    if alt_index in available_indices:
                        print(
                            f"  INFO: '{index}' not available, using '{alt_index}' instead for {name}"
                        )
                ...
                if not replacement_found:
                    ...
                    if available_indices:
                        ...
                        print(
                            f"  INFO: '{index}' not available, using '{fallback}' as fallback for {name}"
                        )
                        globals()["indice_six"] = fallback
                        plot_indices[name] = fallback
                    else:
                        warnings.append(
                            f"Index '{index}' not available for plotting and no alternatives found"
                        )
            else:
    ...
    if missing_plot_indices:
        print(
            "  Note: Some indices were not available but alternatives were used where possible"
        )
    ...
    try:
        ...
        fig_correlation, ax_correlation = plot_correlation_map(
            full_df, R_threshold=0, figsize=(12, 10)
        )
        # Use run identifier for figure naming
        correlation_filename = (
            f"{run_id}_correlation_map.png" if run_id else "correlation_map.png"
        )
        ...
        error_msg = f"Correlation map failed: {e!s}"
    ...
    try:
        ...
        fig, ax[0, 0] = safe_plot_index(
            full_df, plot_indices["indice_one"], ax[0, 0], mode, "indice_one"
        )
        fig, ax[0, 1] = safe_plot_index(
            full_df, plot_indices["indice_two"], ax[0, 1], mode, "indice_two"
        )
        fig, ax[1, 0] = safe_plot_index(
            full_df, plot_indices["indice_three"], ax[1, 0], mode, "indice_three"
        )
        fig, ax[1, 1] = safe_plot_index(
            full_df, plot_indices["indice_four"], ax[1, 1], mode, "indice_four"
        )
        fig, ax[2, 0] = safe_plot_index(
            full_df, plot_indices["indice_five"], ax[2, 0], mode, "indice_five"
        )
        fig, ax[2, 1] = safe_plot_index(
            full_df, plot_indices["indice_six"], ax[2, 1], mode, "indice_six"
        )

        fig.suptitle("Individual Acoustic Features", fontsize=16)
        fig.tight_layout()
        features_filename = (
            f"{run_id}_individual_features.png" if run_id else "individual_features.png"
        )
        ...
        error_msg = f"Individual features plot failed: {e!s}"
    ...
    try:
        ...
        print(
            f"    Per-bin DataFrame columns: {list(result_df_per_bin.columns)[:10]}..."
        )  # Show first 10 columns

        spectro_filename = (
            f"{run_id}_false_color_spectrograms.png"
            if run_id
            else "false_color_spectrograms.png"
        )

        if result_df_per_bin.empty:
            print(
                "    WARNING: Per-bin DataFrame is empty, cannot create false color spectrogram"
            )
            ...
            missing_indices = [
                idx for idx in required_indices if idx not in result_df_per_bin.columns
            ]
            ...
            if missing_indices:
                ...
                per_bin_cols = [
                    col for col in result_df_per_bin.columns if col.endswith("_per_bin")
                ]
                ...
                if len(per_bin_cols) >= 3:
                    ...
                    print(
                        f"    ERROR: Not enough per-bin columns (found {len(per_bin_cols)})"
                    )
                    # Create placeholder
                    fig_spectro, ax = plt.subplots(1, 1, figsize=(16, 14))
                    ax.text(
                    ...
                    fig_spectro.savefig(
                        os.path.join(output_figures_path, spectro_filename)
                    )
                    plt.close(fig_spectro)
                    print(f"    Saved placeholder: {spectro_filename}")
            ...
            print(
                f"    Generating false color spectrogram with indices: {indices_to_use}"
            )
        ...
        error_msg = f"False color spectrogram failed: {e!s}"
    ...
    if errors or warnings:
        message = "Analysis completed with issues:\n\n"
        ...
        error_msg = (
            None
            if len(errors) == 0
            else f"{len(errors)} errors occurred during processing"
        )
...
Label(
    scrollable_frame, text="Input Folder:", font=("Arial", 16), bg="navy", fg="white"
).grid(row=1, column=1, padx=10, pady=10)
Entry(scrollable_frame, textvariable=input_folder_var, font=("Arial", 14)).grid(
...
Button(
    ...
    text="Browse",
    font=("Arial", 16),
    command=lambda: select_folder(input_folder_var),
).grid(row=1, column=3, padx=10, pady=10)

# Output Folder
output_folder_var = StringVar()
Label(
    scrollable_frame, text="Output Folder:", font=("Arial", 16), bg="navy", fg="white"
).grid(row=2, column=1, padx=10, pady=10)
Entry(scrollable_frame, textvariable=output_folder_var, font=("Arial", 14)).grid(
...
Button(
    ...
    text="Browse",
    font=("Arial", 16),
    command=lambda: select_folder(output_folder_var),
).grid(row=2, column=3, padx=10, pady=10)

# Run Identifier (optional - for file naming)
run_identifier_var = StringVar()
Label(
    scrollable_frame, text="Run Identifier:", font=("Arial", 16), bg="navy", fg="white"
).grid(row=3, column=1, padx=10, pady=10)
identifier_entry = Entry(
    scrollable_frame, textvariable=run_identifier_var, font=("Arial", 14)
)
...
Label(
    scrollable_frame, text="Time Scale:", font=("Arial", 16), bg="navy", fg="white"
).grid(row=5, column=1, padx=10, pady=10)
Radiobutton(
    scrollable_frame,
    text="Hourly  ",
    font=("Arial", 16),
    variable=time_scale_var,
    value="hourly",
    bg="navy",
    fg="white",
...
Label(scrollable_frame, text="(secs)", font=("Arial", 16), bg="navy", fg="white").grid(
    row=6, column=4, columnspan=2, padx=5, pady=5
)
Entry(
    scrollable_frame, textvariable=time_interval_var, font=("Arial", 14), width=8
).grid(row=6, column=3, columnspan=1, padx=5, pady=5)

# Separator line
Label(scrollable_frame, text="─" * 50, font=("Arial", 12), bg="navy", fg="gray").grid(
    row=7, column=1, columnspan=4, pady=10
)

# Frequency Band Controls (Optional - for marine acoustics)
Label(
    scrollable_frame,
    text="Acoustic Settings (Optional):",
    font=("Arial", 16, "bold"),
    bg="navy",
    fg="white",
).grid(row=7, column=1, columnspan=3, padx=10, pady=10)
...
Label(
    scrollable_frame,
    text="Anthrophony Range (Hz):",
    font=("Arial", 14),
    bg="navy",
    fg="white",
).grid(row=8, column=1, padx=10, pady=5, sticky="e")
flim_low_entry = Entry(
    scrollable_frame, textvariable=flim_low_var, font=("Arial", 12), width=15
)
...
Label(
    scrollable_frame,
    text="Biophony Range (Hz):",
    font=("Arial", 14),
    bg="navy",
    fg="white",
).grid(row=9, column=1, padx=10, pady=5, sticky="e")
flim_mid_entry = Entry(
    scrollable_frame, textvariable=flim_mid_var, font=("Arial", 12), width=15
)
...
setup_placeholder(
    flim_mid_entry, flim_mid_var, "Default: 1000,10000", flim_mid_placeholder
)
...
Label(
    scrollable_frame, text="Sensitivity (S):", font=("Arial", 14), bg="navy", fg="white"
).grid(row=10, column=1, padx=10, pady=5, sticky="e")
sensitivity_entry = Entry(
    scrollable_frame, textvariable=sensitivity_var, font=("Arial", 12), width=15
)
...
setup_placeholder(
    sensitivity_entry, sensitivity_var, "Default: -35.0", sensitivity_placeholder
)
...
Label(
    scrollable_frame, text="Gain (G):", font=("Arial", 14), bg="navy", fg="white"
).grid(row=11, column=1, padx=10, pady=5, sticky="e")
gain_entry = Entry(
    scrollable_frame, textvariable=gain_var, font=("Arial", 12), width=15
)
...
Label(
    ...
    bg="navy",
    fg="lightgray",
).grid(row=11, column=3, padx=5, pady=5, sticky="w")
setup_placeholder(gain_entry, gain_var, "Default: 0.0", gain_placeholder)

# Performance Settings
Label(scrollable_frame, text="─" * 50, font=("Arial", 12), bg="navy", fg="gray").grid(
    row=13, column=1, columnspan=4, pady=10
)
Label(
    scrollable_frame,
    ...
    bg="navy",
    fg="white",
).grid(row=14, column=1, columnspan=3, padx=10, pady=10)
...
Checkbutton(
    ...
    fg="white",
    selectcolor="darkgrey",
).grid(row=15, column=1, columnspan=2, padx=10, pady=5, sticky="w")
...
progress_label = Label(
    progress_frame, text="", font=("Arial", 12), bg="navy", fg="white"
)
progress_label.pack()
