...
def create_dolphin_clicks(duration, sample_rate=44100):
    ...
    for _ in range(num_clicks):
        ...
        if click_idx < len(signal) - 100:
            ...
            # Add click to signal
            signal[click_idx : click_idx + click_samples] += click[
                : min(click_samples, len(signal) - click_idx)
            ]
...
def create_test_dataset(output_dir="test_wav_files", num_files=5, duration=30):
    ...
    for i, scenario in enumerate(scenarios[:num_files]):
        ...
        filename = (
            f"marine_{timestamp.strftime('%Y%m%d_%H%M%S')}_{scenario['name']}.wav"
        )
    ...
    print("3. Choose an test_outputs folder for results")
    print("4. Run the analysis to see acoustic indices and visualizations")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create sample marine acoustic WAV files for testing"
    )
    parser.add_argument(
        "--test_outputs",
        "-o",
        default="test_wav_files",
    ...
    create_test_dataset(args.test_outputs, args.num_files, args.duration)
