...
class RunMetadata:
    ...
    def add_file_info(self, input_files, output_files):
        ...
        self.metadata["file_manifest"] = {
            "input_files": {
                ...
                "files": [
                    {"path": f, "size": self._get_file_size(f)} for f in input_files
                ],
                "total_size_mb": sum(self._get_file_size(f) for f in input_files)
                / (1024 * 1024),
            },
            "output_files": {
    ...
    def _save_metadata(self):
        ...
        try:
            ...
            run_identifier = (
                self.metadata["input_settings"].get("run_identifier", "").strip()
            )
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def _save_summary_report(self, filepath):
        ...
        try:
            with open(filepath, "w") as f:
                ...
                f.write(
                    f"Duration: {run_info.get('duration_seconds', 0):.1f} seconds\n"
                )
                ...
                f.write(
                    f"Output Folder: {input_settings.get('output_folder', 'N/A')}\n"
                )
                f.write(f"Processing Mode: {input_settings.get('mode', 'N/A')}\n")
                f.write(
                    f"Frequency Bands: Anthro {input_settings.get('flim_low', 'N/A')}, Bio {input_settings.get('flim_mid', 'N/A')}\n"
                )
                f.write(
                    f"Parallel Processing: {'Yes' if input_settings.get('parallel_enabled') else 'No'}\n"
                )
                ...
                if file_info:
                    ...
                    f.write(
                        f"Total Input Size: {input_files.get('total_size_mb', 0):.1f} MB\n"
                    )
                ...
                if perf_info:
                    ...
                    if "sequential_time" in perf_info and "parallel_time" in perf_info:
                        f.write(
                            f"Sequential Time: {perf_info['sequential_time']:.1f}s\n"
                        )
                        f.write(f"Parallel Time: {perf_info['parallel_time']:.1f}s\n")
                        speedup = (
                            perf_info["sequential_time"] / perf_info["parallel_time"]
                        )
                        f.write(f"Speedup: {speedup:.1f}x\n")
                    elif "processing_time" in perf_info:
                        f.write(
                            f"Processing Time: {perf_info['processing_time']:.1f}s\n"
                        )
...
if __name__ == "__main__":
    ...
    metadata.add_processing_info(files_found=10, processing_mode="parallel")

    # Add performance metrics
    metadata.add_performance_metrics(
        sequential_time=45.2, parallel_time=12.8, files_processed=10, files_failed=0
    )
